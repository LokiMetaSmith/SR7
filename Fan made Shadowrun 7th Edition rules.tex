...
\begin{itemize}[nosep]
\item \textbf{Dice Pool Bonus:} The Rigger adds their Control Rig rating to all Vehicle skill checks (Piloting, Gunnery, Sensors).
\item \textbf{Threshold Reduction:} The Threshold for any Vehicle test is reduced by the Control Rig rating (to a minimum of 1).
\item \textbf{Initiative:} While jumped in, the Rigger uses Matrix Initiative (Data Processing + Intuition) and gains +1 Initiative Die per level of their Control Rig.
\end{itemize}
...
\begin{itemize}[nosep]
\item If the vehicle takes Physical damage, the Rigger must resist half of that damage (rounded down) as Stun Biofeedback. This is resisted with \gameterm{WIL + BOD}.
\item If the vehicle's Condition Monitor is filled and it is destroyed while the Rigger is jumped in, the Rigger suffers severe Dumpshock, immediately taking 6 unresistable Stun damage and suffering a -2 Dice Pool modifier to all actions for (12 - WIL) minutes.
\end{itemize}

\textbf{Chase Combat}\\
Chases are high-stakes affairs that abstract exact speeds into Range Bands: Close, Short, Medium, Long, and Extreme.
\begin{itemize}[nosep]
\item \textbf{The Chase Turn:} At the start of a Combat Round where a chase is occurring, all participating drivers make an Opposed \gameterm{Pilot + Reaction} test. The net hits determine if a vehicle moves up or down a Range Band relative to their target.
\item \textbf{Chase Maneuvers:} Drivers can spend their Complex actions during their Initiative pass to attempt specific maneuvers:
    \begin{itemize}[nosep]
    \item \emph{Catch Up / Break Away:} An opposed Pilot test to force a shift in Range Band.
    \item \emph{Evade:} The driver adds their Vehicle's Handling rating to their Defense Pool against incoming attacks for the round.
    \item \emph{Ram:} An opposed Pilot test. If successful, both vehicles take Physical damage based on the ramming vehicle's Body + Speed. However, the vehicle with the higher Body + Armor resists the damage with double their dice pool, and halves the final damage taken (ensuring heavier vehicles win crashes).
    \item \emph{Cut Off:} Attempted at Close or Short range. An opposed Pilot test to force the target vehicle to make a Crash Test.
    \end{itemize}
\end{itemize}
