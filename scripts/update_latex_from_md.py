...
def main():
    parser = argparse.ArgumentParser(
        description="Update LaTeX tables from Markdown source."
    )
    parser.add_argument(
        "--md",
    ...
    parser.add_argument(
        "--tex",
        default="Fan made Shadowrun 7th Edition rules.tex",
    ...
    lt_pattern = re.compile(
        r"\\begin\{longtable\}\[\]\{@\{\}(.*?)\\endhead\s*\\bottomrule\\noalign\{\}\s*\\endlastfoot\n(.*?)(\\end\{longtable\})",
        re.DOTALL,
    )
    ...
    def replacer(match):
        ...
        tex_headers = re.findall(
            r"\\begin\{minipage\}.*?\\raggedright\s*(.*?)\s*\\end\{minipage\}",
            header_def,
            re.DOTALL,
        )
        if not tex_headers:
    ...
    if new_tex_content != tex_content:
        ...
        print(
            "No changes made. Either tables already match or couldn't find matching headers."
        )
